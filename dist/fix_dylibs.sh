...
while ! [ "$LIST" = "" ]; do
	...
	for FILE in $LIST; do
		...
		for FLIB in $($1-otool -L $FILE | grep -v : | grep libwx | cut -d '(' -f 1); do 
            NLIB=$(echo $FLIB|rev|cut -d '/' -f 1|rev)
			echo "    found $NLIB"
            $1-install_name_tool -change $FLIB @executable_path/../Frameworks/$NLIB $FILE
            if ! [ -f Frameworks/$NLIB ]; then 
				cp $WXDIR/lib/$NLIB Frameworks/ 
				NEXT_LIST="$NEXT_LIST Frameworks/$NLIB"
			fi
		done
# 		for FLIB in $($1-otool -L $FILE | grep /opt/mac | cut -d '(' -f 1); do 
# 			NLIB=$(echo $FLIB|rev|cut -d '/' -f 1|rev)
# 			echo "    found $NLIB"
# 			$1-install_name_tool -change $FLIB @executable_path/../Frameworks/$NLIB $FILE
# 			if ! [ -f Frameworks/$NLIB ]; then 
