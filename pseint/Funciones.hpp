...
struct Funcion {
	...
	int cant_arg = -1; // cantidad de argumentos (sin contar el valor de retorno, los argumentos seran nombres[1...cant_arg])
	...
	Memoria *memoria = nullptr; // instancia de la clase memoria que se usa para el analisis sintático
	Funcion(int line) : line_start(line) { AddArg(""); }
	// cppcheck-suppress uninitMemberVar
	Funcion(tipo_var tipo_ret, FunctionPtr af):cant_arg(0),func(af) { 
		tipos.resize(cant_arg+1); tipos[0]=tipo_ret;
		pasajes.resize(cant_arg+1); pasajes[0]=PP_DESCONOCIDO;
	}
	// cppcheck-suppress uninitMemberVar
	Funcion(tipo_var tipo_ret, FunctionPtr af, tipo_var tipo_arg_1):cant_arg(1),func(af) { 
		tipos.resize(cant_arg+1); tipos[0]=tipo_ret; tipos[1]=tipo_arg_1;
		pasajes.resize(cant_arg+1); pasajes[0]=pasajes[1]=PP_DESCONOCIDO;
	}
	// cppcheck-suppress uninitMemberVar
	Funcion(tipo_var tipo_ret, FunctionPtr af, tipo_var tipo_arg_1, tipo_var tipo_arg_2):cant_arg(2),func(af) { 
	...
	// cppcheck-suppress uninitMemberVar
	Funcion(tipo_var tipo_ret, FunctionPtr af, tipo_var tipo_arg_1, tipo_var tipo_arg_2, tipo_var tipo_arg_3):cant_arg(3),func(af) { 
